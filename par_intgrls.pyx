...
@cython.boundscheck(False)
@cython.wraparound(False)
def M_times_N_f_intgrls_raw_no_func(double[:, ::1] fxs, double[::1] ux, double[::1] iiq2, double dSilenceX, double[::1] px, double[:, ::1] f_exp_px, int M, int fss, int Nupx, int nthrds):
    #  fxs       M x fss   
    #  fxrux     Nupx    
    #  f_intgrd  Nupx
    cdef int fi, m, n, mfss
    cdef double dd, IIQ2, tot
    ...
    with nogil, parallel(num_threads=nthrds):
        for m in prange(M):
            ...
            for fi in xrange(fss):
                mfss = m*fss
                p_f_exp_px[mfss + fi] =  0

                for n in xrange(Nupx):
                    dd = p_fxs[fss + fi]-p_ux[n]
                    p_f_exp_px[mfss+fi] += exp(-0.5*dd*dd*IIQ2)*p_px[n]
                p_f_exp_px[mfss+fi] *= dSilenceX




#intgrl(m, p_fxs, p_ux, p_px, M, fss, Nupx, dSilenceX, IIQ2, fi)
    #     #  f_exp_px   is M x fss
