...
for an in animals[0:1]:
    ...
    for day in xrange(3, 4):
        ...
        if os.access("%s" % frip, os.F_OK):
            ...
            for epc in range(0, _pts.shape[1]+1, 2):
                ep=epc-1;
            ...
            # frwp = "%(bd)s/Dropbox (EastWestSideHippos)/BostonData/%(s3)s/%(s1)srawpos%(sdy)s.mat" % {"s1" : anim1, "sdy" : sdy, "s3" : anim3, "bd" : basedir}
                ...
                if (_pts.shape[1] > 0):     #  might be empty epoch
                    ...
                    nzinds = _N.where(r[:, 1] > 0)[0]   #  not occluded data
                    ...
                    for ri in xrange(1, len(real_arm_events)):
                        ...
                        if debugFigs:
                            btwnfigs(ep, t0, t1, seg_ts, r, x, y)
                    ...
                    if debugFigs:
                        piecesfigs(ep, lstrans, tstrans, r, scxMin, scxMax, scyMin, scyMax)
