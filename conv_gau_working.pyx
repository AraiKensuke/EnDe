...
def build_gau_pxdx_tbl(px, x_grid, x_path, fs, iq2s, i0, k, f_L, f_H, usepx=True):
    ...
    for f in fs:
        ...
        for iq2 in iq2s:
            qi += 1
            if usepx:
                p_tab[fi*_Nq2 + qi] = -sqrt(iq2)*_isqrt2pi*_N.sum(_N.exp(-0.5*dd2*iq2)*px)
            else:
                p_tab[fi*_Nq2 + qi] = -sqrt(iq2)*_isqrt2pi*_N.sum(_N.exp(-0.5*dd2*iq2))
...
cdef double conv_px(double f, double q) nogil:
    global _k, _i0, _Nf, _f_L, _f_H, _tab, p_tab, _Nfm1, _2ik, _f_H_m__f_L
