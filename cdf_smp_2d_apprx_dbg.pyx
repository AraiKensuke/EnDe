...
def smp_f(int itr, int M, double[::1] x_or_yt0t1, long[::1] clstsz, long[::1] cls_strt_inds, 
...
@cython.boundscheck(False)
@cython.wraparound(False)
def smp_q2(int itr, int M, double[::1] x_or_yt0t1, long[::1] clstsz, long[::1] cls_strt_inds, 
           long[::1] sts, int t0, double[::1] l0, 
...
cdef double pdfIG(int totalpcs, double dt, double l0, double a, double B, double q2_smpld, double *p_path_sd2, double *p_diffs1, double *p_nrm1, double *p_inv_sum_sd2s1, double *p_diffs2) nogil: 
    ...
    for 0 <= pcs < totalpcs:
        ...
        sptlIntgrl += 1.p_nrm1[pcs] / sqrt(twpi*sum_vars)*exp(-0.5*p_diffs1[pcs]*p_inv_sum_sd2s1[pcs] - 0.5*p_diffs2[pcs] / sum_vars)
    sptlIntgrl *= dt*l0
