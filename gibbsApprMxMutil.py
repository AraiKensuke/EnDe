...
def finish_epoch2_2d(oo, nSpks, epc, ITERS, gz, l0, fx, fy, q2x, q2y, u, Sg, _fx_u, _fy_u, _fx_q2, _fy_q2, _q2x_a, _q2y_a, _q2x_B, _q2y_B, _l0_a, _l0_B, _u_u, _u_Sg, _Sg_nu, _Sg_PSI, smp_sp_prms, smp_mk_prms, smp_mk_hyps, freeClstr, M_use, K, priors, m1stSignalClstr, ):
    ...
    if oo.resetClus and (M_use > 1):
        for m in xrange(M_use):
            ...
            if breset:
                print "****RESET cluster %(m)d  with o %(o)f" % {"m" : m, "o" : occ[m]}
                reset_cluster_2d(epc, m, l0, fx, fy, q2x, q2y, freeClstr, _q2x_a, _q2y_a, _q2x_B, _q2y_B, _fx_u, _fy_u, _fx_q2, _fy_q2, _l0_a, _l0_B, _u_u, _u_Sg, _Sg_nu, _Sg_PSI, oo, priors, m1stSignalClstr)
            else:
...
def reset_cluster_2d(epc, m, l0, fx, fy, q2x, q2y, freeClstr, _q2x_a, _q2y_a, _q2x_B, _q2y_B, _fx_u, _fy_u, _fx_q2, _fy_q2, _l0_a, _l0_B, _u_u, _u_Sg, _Sg_nu, _Sg_PSI, oo, priors, m1stSignalClstr):
...
def contiguous_inuse_2d(M_use, M_max, K, freeClstr, l0, fx, fy, q2x, q2y, u, Sg, _l0_a, _l0_B, _fx_u, _fy_u, _fx_q2, _fy_q2, _q2x_a, _q2y_a, _q2x_B, _q2y_B, _u_u, _u_Sg, _Sg_nu, _Sg_PSI, smp_sp_prms, smp_mk_prms, sp_prmPstMd, mk_prmPstMd, gz, priors):
    #  method called after Gibbs iters are completed
    #  work only with small parameter array, not Master.
    #  _l0_a  (for example) are posterior (to become prior) hyp params calculated from l0s
    ...
    if len(freeIDs > 0):
        ...
        temp5   = _N.empty((5, smp_sp_prms.shape[1]))
        tempK   = _N.empty((K, smp_sp_prms.shape[1]))
        tempKxK = _N.empty((K, K, smp_sp_prms.shape[1]))
        #  IDs in use past the 
        inuseIDs = _N.where(freeClstr[mf+1:M_use] == False)[0] + mf + 1
