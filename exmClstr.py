...
def timeline_v2(bfn, datfn, itvfn, outfn="timeline", ch1=0, ch2=1, xL=0, xH=3, yticks=[0, 1, 2, 3], thin=1, allFR=None, tcksz=19, lblsz=21):   #  25Hz
    d = _N.loadtxt(datFN("%s.dat" % datfn))   #  marks
    itv = _N.loadtxt(datFN("%s.dat" % itvfn))
    N = d.shape[0]          #  ALL spikes
    epochs = itv.shape[0]-1
    ...
    mF.arbitraryAxes(ax, axesVis=[True, True, False, False], xtpos="bottom", ytpos="left")
    mF.setTicksAndLims(xlabel=None, ylabel="elctrd 2\nmark (a.u.)", xticks=None, yticks=[], xticksD=None, yticksD=None, xlim=[0, N/1000.], ylim=[wvfmMin[1], wvfmMax[1]], tickFS=tcksz, labelFS=lblsz)
    for ep in xrange(epochs):
        _plt.axvline(x=(itv[ep+1]*N/1000.), color="red", ls="--")
    #######################
    ...
    mF.arbitraryAxes(ax, axesVis=[True, True, False, False], xtpos="bottom", ytpos="left")
    mF.setTicksAndLims(xlabel=None, ylabel="elctrd 3\nmark (a.u.)", xticks=None, yticks=[], xticksD=None, yticksD=None, xlim=[0, N/1000.], ylim=[wvfmMin[0], wvfmMax[0]], tickFS=tcksz, labelFS=lblsz)
    for ep in xrange(epochs):
        _plt.axvline(x=(itv[ep+1]*N/1000.), color="red", ls="--")
    #######################
    ...
    mF.setTicksAndLims(xlabel="time (s)", ylabel="elctrd 4\nmark (a.u.)", xticks=None, yticks=[], xticksD=None, yticksD=None, xlim=[0, N/1000.], ylim=[wvfmMin[1], wvfmMax[1]], tickFS=tcksz, labelFS=lblsz)
    for ep in xrange(epochs):
        _plt.axvline(x=(itv[ep+1]*N/1000.), color="red", ls="--")
    ...
    fig.subplots_adjust(left=0.08, bottom=0.08, right=0.95, top=0.95, wspace=0.4, hspace=0.4)
    epochs = len(itv)-1
    for fmt in ["png", "eps"]:
